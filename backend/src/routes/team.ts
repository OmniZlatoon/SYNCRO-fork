 ...
 */
async function resolveUserTeam(userId: string): Promise<{
  teamId: string;
  isOwner: boolean;
  ...
  try {
    ...
    if (!ctx) {
      ...
      if (createErr || !newTeam)
        throw createErr ?? new Error("Failed to create team");
    ...
    if (alreadyMember) {
      return res
        .status(409)
        .json({ success: false, error: "This user is already a team member" });
    }
    ...
    if (invErr || !invitation)
      throw invErr ?? new Error("Failed to create invitation");
...
router.get("/pending", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!canManageTeam(ctx)) {
      return res.status(403).json({
        success: false,
        error: "Only team owners and admins can view pending invitations",
      });
    }
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to list pending invitations",
    });
  }
});
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!ctx) {
        return res
          .status(403)
          .json({ success: false, error: "You are not part of a team" });
      }
      ...
      if (team?.owner_id === member.user_id) {
        return res
          .status(400)
          .json({ success: false, error: "Cannot remove the team owner" });
      }
